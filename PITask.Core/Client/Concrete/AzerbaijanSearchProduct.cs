...
{
    public class AzerbaijanSearchProduct : SearchProduct
    {
        protected override void search(object sender, int id)
        {
            var product = CustomHttpClient.GetResponse("Azerbaijan", id).Result;
        ...
        {
            return CustomHttpClient.GetResponse("Azerbaijan", id).Result;
        }
    }
}
